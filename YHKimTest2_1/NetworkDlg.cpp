...
void CNetworkDlg::OnReceive() {
	...
	{
		pBuf[iRcvd] = NULL;
		CString str = (LPCWSTR(pBuf));
		CString x1, y1, x2, y2;
		AfxExtractSubString(x1, str, 0, ',');
		AfxExtractSubString(y1, str, 1, ',');
		AfxExtractSubString(x2, str, 2, ',');
		AfxExtractSubString(y2, str, 3, ',');
		m_x1 = _ttoi(x1);
		m_y1 = _ttoi(y1);
		m_x2 = _ttoi(x2);
		m_y2 = _ttoi(y2);
...
void CNetworkDlg::OnBnClickedBenter()
{
	...
	int iSent, iLen;
	CString str;
	str.Format(_T("%d,%d,%d,%d"), setPoint[0].x, setPoint[0].y, setPoint[1].x, setPoint[1].y);
	iLen = str.GetLength();
	iSent = m_Csocket.Send(LPCTSTR(str), sizeof(TCHAR)*(iLen + 1));
